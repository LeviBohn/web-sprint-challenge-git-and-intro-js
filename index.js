...
function getArtistByIndex(array, atIndex) {
  return `the artist at index ${array[atIndex].id} is ${array[atIndex].name}`
  // return 'the artist at ' + index array[atIndex].id + " is " + array[atIndex].name;
}
...
function removeArtist(array, toBeRemoved) {
  const copiedArray = array.slice();
  copiedArray.splice(copiedArray[toBeRemoved],1);
  return copiedArray;
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  const copiedArray = array.slice();
  copiedArray.push({name, years, genre, nationality, bio});
  return copiedArray;
}
console.log ('task 6:', addArtist(artists, 'Levi', '1980-2022', 'Web Design', 'Anglo American', 'I sold cars for several years. Now I want to create software to sell to that industry.'));
...
function lotsOfArt(array) {
  ...
  for (let i=0; i<array.length; i++){
    if (array[i].paintings > 100){
      superArtists.push(array[i].name);
    }
  }
  return superArtists;
}
